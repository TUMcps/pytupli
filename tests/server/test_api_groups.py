...
async def create_user(client, headers, username):
    return await client.post(
        '/access/users/create', json={'username': username, 'password': 'test1234'}, headers=headers
    )
...
async def test_list_groups_standard_user_with_membership(
    async_client, admin_headers, standard_user1_headers
):
    group = group_data()
    await create_group(async_client, admin_headers, group)
    ...
    membership_query = GroupMembershipQuery(
        group_name=group.name, members=[GroupMembership(user='test_user_1', roles=['test_role'])]
    )
    await async_client.post(
        '/access/groups/add-members', json=membership_query.model_dump(), headers=admin_headers
    )
    try:
        resp = await async_client.get('/access/groups/list', headers=standard_user1_headers)
        assert resp.status_code == 200
...
async def test_read_group_success(async_client, admin_headers):
    ...
    try:
        resp = await async_client.get(
            f'/access/groups/read?group_name={group.name}', headers=admin_headers
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data['name'] == group.name
...
async def test_read_group_not_found(async_client, admin_headers):
    resp = await async_client.get(
        '/access/groups/read?group_name=nonexistent_group', headers=admin_headers
    )
...
async def test_read_group_unauthorized(async_client, standard_user1_headers):
    group = group_data()
    resp = await async_client.get(
        f'/access/groups/read?group_name={group.name}', headers=standard_user1_headers
    )
...
async def test_add_members_success(async_client, admin_headers):
    ...
    membership_query = GroupMembershipQuery(
        group_name=group.name, members=[GroupMembership(user='member_user', roles=['test_role'])]
    )
...
async def test_add_members_no_roles_skipped(async_client, admin_headers):
    ...
    await create_user(async_client, admin_headers, 'member_user')
    membership_query = GroupMembershipQuery(
        group_name=group.name, members=[GroupMembership(user='member_user', roles=[])]
    )
    resp = await async_client.post(
        '/access/groups/add-members', json=membership_query.model_dump(), headers=admin_headers
    )
...
async def test_add_members_unauthorized(async_client, standard_user1_headers):
    group = group_data()
    membership_query = GroupMembershipQuery(
        group_name=group.name, members=[GroupMembership(user='test_user', roles=['test_role'])]
    )
    resp = await async_client.post(
        '/access/groups/add-members',
        json=membership_query.model_dump(),
        headers=standard_user1_headers,
    )
    assert resp.status_code == 403
...
async def test_add_members_replaces_existing_membership(async_client, admin_headers):
    ...
    test_role2 = {
        ...
        'rights': [RIGHT.ARTIFACT_CREATE],
    }
    await create_role(async_client, admin_headers, test_role1)
    await create_role(async_client, admin_headers, test_role2)
    await create_user(async_client, admin_headers, 'member_user')
    membership_query = GroupMembershipQuery(
        group_name=group.name, members=[GroupMembership(user='member_user', roles=['test_role1'])]
    )
    await async_client.post(
        '/access/groups/add-members', json=membership_query.model_dump(), headers=admin_headers
    )
    membership_query = GroupMembershipQuery(
        group_name=group.name, members=[GroupMembership(user='member_user', roles=['test_role2'])]
    )
    resp = await async_client.post(
        '/access/groups/add-members', json=membership_query.model_dump(), headers=admin_headers
    )
...
async def test_remove_members_success(async_client, admin_headers):
    ...
    assert len(read_data['members']) == 2
    membership_query = GroupMembershipQuery(
        group_name=group.name, members=[GroupMembership(user='member_user')]
    )
    resp = await async_client.post(
        '/access/groups/remove-members', json=membership_query.model_dump(), headers=admin_headers
    )
    assert resp.status_code == 200
    read_resp = await async_client.get(
        f'/access/groups/read?group_name={group.name}', headers=admin_headers
...
async def test_remove_members_group_not_found(async_client, admin_headers):
    membership_query = GroupMembershipQuery(
        group_name='nonexistent_group', members=[GroupMembership(user='test_user')]
    )
    resp = await async_client.post(
        '/access/groups/remove-members', json=membership_query.model_dump(), headers=admin_headers
    )
...
async def test_remove_members_user_not_found(async_client, admin_headers):
    ...
    await create_group(async_client, admin_headers, group)
    membership_query = GroupMembershipQuery(
        group_name=group.name, members=[GroupMembership(user='nonexistent_user')]
    )
    resp = await async_client.post(
        '/access/groups/remove-members', json=membership_query.model_dump(), headers=admin_headers
    )
...
async def test_remove_members_unauthorized(async_client, standard_user1_headers):
    group = group_data()
    membership_query = GroupMembershipQuery(
        group_name=group.name, members=[GroupMembership(user='test_user')]
    )
    resp = await async_client.post(
        '/access/groups/remove-members',
        json=membership_query.model_dump(),
        headers=standard_user1_headers,
    )
    assert resp.status_code == 403
...
async def test_group_auth_required_no_headers(async_client):
    ...
    assert resp.status_code == 403
    resp = await async_client.post(
        '/access/groups/remove-members', json=membership_query.model_dump()
    )
    assert resp.status_code == 403


async def test_group_creator_can_manage_members(
    async_client, admin_headers, standard_user1_headers, standard_user2_headers
):
    ...
    try:
        ...
        membership_query = GroupMembershipQuery(
            group_name=group.name, members=[GroupMembership(user='test_member_user')]
        )
        resp = await async_client.post(
            '/access/groups/remove-members',
            json=membership_query.model_dump(),
        ...
        resp = await async_client.post(
            '/access/groups/add-members',
            json=membership_query.model_dump(),
            headers=standard_user2_headers,
        )
        assert resp.status_code == 403
