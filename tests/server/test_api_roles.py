...
def test_role_data():
    return UserRole(role='test_role', description='A test role', rights=[RIGHT.ARTIFACT_READ])


async def test_create_role_success(async_client, admin_headers, test_role_data):
    resp = await async_client.post(
        '/access/roles/create', json=test_role_data.model_dump(), headers=admin_headers
    )
    assert resp.status_code == 200
    assert resp.json()['role'] == test_role_data.role
    # Cleanup
    await async_client.delete(
        f'/access/roles/delete?role_name={test_role_data.role}', headers=admin_headers
    )
...
async def test_delete_role_not_found(async_client, admin_headers):
    resp = await async_client.delete(
        '/access/roles/delete?role_name=nonexistent_role', headers=admin_headers
    )
    assert resp.status_code == 200
...
async def test_role_auth_required(async_client, standard_user1_headers, test_role_data):
    ...
    resp = await async_client.post(
        '/access/roles/create', json=test_role_data.model_dump(), headers=standard_user1_headers
    )
    assert resp.status_code == 403
    resp = await async_client.get('/access/roles/list', headers=standard_user1_headers)
    assert resp.status_code == 403
    resp = await async_client.delete(
        f'/access/roles/delete?role_name={test_role_data.role}', headers=standard_user1_headers
    )
    assert resp.status_code == 403
...
async def test_delete_role_removes_from_user(async_client, admin_headers, test_role_data):
    ...
    await async_client.post(
        '/access/users/create',
        json={'username': 'role_user', 'password': 'test1234'},
    ...
    await async_client.put(
        '/access/users/update?username=role_user', json=update, headers=admin_headers
    )
    # Delete role
    resp = await async_client.delete(
        f'/access/roles/delete?role_name={test_role_data.role}', headers=admin_headers
